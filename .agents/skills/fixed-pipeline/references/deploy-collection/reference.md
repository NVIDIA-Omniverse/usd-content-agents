...
description: "Fixed pipeline reference: Deploy the Content Agents REST service collection with a provider-neutral, endpoint-driven, multi-instance strategy. Use when the user explicitly asks to deploy all fixed pipeline services, configure shared endpoints, test the service package, or use Brev for self-hosted dependencies."
version: "0.1.0"
...
# Fixed Pipeline Reference: Deploy Content Agents Collection

This deploys the explicit fixed pipeline REST service collection. It does not replace
or host the default agentic Content Workflow.
