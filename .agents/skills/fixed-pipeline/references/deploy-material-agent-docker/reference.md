...
description: "Fixed pipeline reference: Deploy the material-agent-service locally using Docker Compose with OVRTX rendering. Use when the user explicitly requests Material Agent Docker, Compose, local REST deployment, GPU rendering containers, or service-side VLM configuration; use the agentic Content Workflow for unqualified material tasks."
version: "0.1.1"
...
# Fixed Pipeline Reference: Deploy Material Agent Service with Docker Compose
...
- Use the root `$quickstart` for the default Agentic first run. Continue with
  ...
  `deploy-collection` when running multiple Content Agents together with
  shared dependency endpoints.
