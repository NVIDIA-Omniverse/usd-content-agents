...
description: "Fixed pipeline reference: Deploy the physics-agent-service locally using Docker Compose with its OVRTX GPU sidecar. Use when the user explicitly requests Physics Agent Docker, Compose, REST deployment, service-side VLM configuration, or tune/refine routes; use the agentic Content Workflow for unqualified physics tasks."
version: "0.1.2"
...
# Fixed Pipeline Reference: Deploy Physics Agent Service with Docker Compose
...
| `PA_VLM_MODEL` | `moonshotai/kimi-k3` | Model id for the selected backend |
