...
description: "Fixed pipeline reference: Deploy the texture-agent-service with Docker Compose, including hosted/simple image generation, optional local sidecars, and a Texture Variation API overlay. Use when the user explicitly requests Texture Agent Docker, Compose, REST deployment, Step1X-compatible service validation, or local sidecars; use the agentic Content Workflow for unqualified texture tasks."
version: "0.3.2"
...
# Fixed Pipeline Reference: Deploy Texture Agent Service with Docker Compose

This is a fixed pipeline service deployment, not the default agentic
Content Workflow.
...
- Use the root `$quickstart` for the default Agentic first run. Continue with
  this reference only for an explicitly requested fixed pipeline service; use
  `deploy-collection` when running multiple Content Agents together.
...
- **Default:** hosted image generation and hosted auto-prompt LLM.
- **Local NIM sidecars:** optional `image-gen` and `llm` Compose profiles plus
  the multi-GPU overlay.
- **External Texture Variation service:** the main service calls an
  operator-provided compatible endpoint.

### 2. Configure the Environment

Default NVIDIA-hosted backend:
...
`HF_TOKEN` is required by the image-generation NIM profile; the LLM-only
profile needs `NGC_API_KEY` but not `HF_TOKEN`.

External Texture Variation API:
...
Do not put secrets directly in Compose files, shell history, or chat. Use an
ignored `.env` file or the deployment platform's secret store.

### 3. Start the Default Service
...
Compose interpolation reads the repository-root `.env` only when
`--env-file .env` is passed. The service is available at
`http://localhost:8001`.

### 4. Start Optional Local NIM Sidecars

The NIM containers read `NGC_API_KEY` and `HF_TOKEN` from the ignored
repository-root `.env`. Authenticate Docker to NGC separately, supplying
`NGC_API_KEY` from your shell's secret source without exposing it in process
arguments:
...
Start both local profiles with the overlay that rewrites their service URLs:

```bash
docker compose --env-file .env \
  -f apps/texture_agent_service/docker-compose.yml \
  -f apps/texture_agent_service/docker-compose.multi-gpu.yml \
  --profile image-gen --profile llm \
  up --build -d
```
...
- selected mode and Compose files;
- service URL and `/health` result;
- backend type and endpoint hostname, without credentials;
- running containers and any unhealthy service;
- one concise next action when startup or routing fails.

Never report API keys, tokens, full secret-bearing URLs, or `.env` contents.
...
### Service Cannot Reach an External Texture Variation API
...
### Local NIM Profile Starts but Traffic Still Uses Hosted Backends

The overlay must use the Docker DNS endpoint from inside the service:

### Port 8001 Is Busy

Stop the conflicting service or adjust the published port in a local Compose
override. Keep the internal container port at `8001`.

### GPU Sidecar Is Unhealthy
