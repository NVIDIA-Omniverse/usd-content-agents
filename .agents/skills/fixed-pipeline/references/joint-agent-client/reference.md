...
description: "Fixed pipeline reference: Make requests to the Joint Agent REST API for articulation classification. Use when the user explicitly requests the fixed pipeline service/client, uploads, REST monitoring, prediction/report downloads, or a client script; use the reviewed agentic Content Workflow for unqualified articulation tasks."
version: "0.1.0"
...
# Fixed Pipeline Reference: Joint Agent Client

This is the explicit fixed pipeline REST route, not the default reviewed agentic
Content Workflow.
...
curl -fL -o rigged.usdz "$BASE_URL/artifacts/$SESSION/joint-rigger-output"
```
