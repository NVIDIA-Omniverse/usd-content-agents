...
description: "Fixed pipeline reference: Run the direct Material Agent CLI for config-driven VLM material assignment. Use when the user explicitly requests material-agent commands, a material-agent config YAML, fixed pipeline resume behavior, material benchmarks, dataset construction, predicted-material application, or a SimReady demo; use the agentic Content Workflow for unqualified material tasks."
version: "0.1.1"
...
# Fixed Pipeline Reference: Material Agent CLI
...
- Use when the user explicitly wants the fixed pipeline predict/apply/render/judge
  loop from a Material Agent config.
...
- Use when the user wants a public SimReady demo asset with minimal local data
  setup.
- For an unqualified material task or an interactive agentic asset workflow,
  use the root `content-workflow-material` and
  `content-workflow-cli` skills instead. This skill is the explicit fixed pipeline
  route.
...
- On WSL2, install and initialize the supported Warp runtime with
  `uv pip install -e ".[warp]" -e apps/material_agent` and
  `python -c "import newton; import warp as wp; wp.init()"`. Copy the example
  config beside the original and set both
  `steps.build_dataset_usd.renderer.backend` and `steps.render.backend` to
  `warp`; do not run the unedited remote-rendering template.
...
2. Choose a config. For a first native-Linux run, use
   `apps/material_agent/configs/unified_example.yaml`. On WSL2, copy that config
   beside the original and apply both Warp renderer changes from Prerequisites.
   For a user asset, copy that config or use `material-agent configure`.
