...
This reference is shared with the `$fixed-pipeline` umbrella's
`physics-agent-cli` and `texture-agent-cli` references; the variant table below
picks the right USD flavor for each agent.
...
```bash
wu chat "hi" --backend nim --model moonshotai/kimi-k3 --max-tokens 8
```
