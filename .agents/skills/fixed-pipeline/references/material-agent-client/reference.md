...
description: "Fixed pipeline reference: Make requests to the Material Agent REST API service for VLM material assignment. Use when the user explicitly wants the fixed pipeline Material Agent client or service API, uploads, REST monitoring, generated references, result downloads, or a client script; use the agentic Content Workflow for unqualified material tasks."
version: "0.1.2"
...
# Fixed Pipeline Reference: Material Agent Client
...
This is the explicit fixed pipeline REST route. For an unqualified material task, use
the root `content-workflow-material` and `content-workflow-cli` skills instead.
...
- Use `deploy-material-agent-docker` first when no Material Agent service is
  running yet.
...
3. Provide `user_email` only when the caller has one for tracking. When it is
   ...
   `anonymous@example.com` if that fallback is blank. Provide exactly one input
...
From the repository root, expose that reference module before running the
Python example:

```bash
export PYTHONPATH="$PWD/.agents/skills/fixed-pipeline/references/material-agent-client/references${PYTHONPATH:+:$PYTHONPATH}"
```
...
| `user_email` | No | Optional tracking email. Omitted or blank uses `MA_DEFAULT_USER_EMAIL`, then `anonymous@example.com`. |
