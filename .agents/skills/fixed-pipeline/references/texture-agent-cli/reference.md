...
description: "Fixed pipeline reference: Run the Texture Agent CLI for config-driven AI texture generation on USD materials. Use when the user explicitly requests texture-agent, YAML/config control, its step sequence, Python-compatible artifacts, or fixed pipeline resume behavior; use content-workflow-cli texture for unqualified texture tasks."
version: "0.1.0"
...
# Fixed Pipeline Reference: Texture Agent CLI
...
- Use when the user wants to discover available materials before writing
  prompts.
- For an unqualified texture task, use `content-workflow-texture` and its
  focused `texture prepare` through `texture publish` operations instead.
...
- `remote` uses `RENDER_ENDPOINT`; `ovrtx` uses the local isolated renderer;
  ...
  evidence.
- Final remote rendering explicitly confines bundle inputs to the run working
  directory. This supports the generated sibling `output/` and `textures/`
  directories without allowing traversal, unrelated siblings, or symlink
  escapes outside that run root.
...
For a quick before/after render, use two named `usd-cli` sessions so the source
and textured output remain separate and both renders use the configured local
or remote OVRTX backend:

```bash
usd-cli --session texture-before open <input.usd>
usd-cli --session texture-before render --renderer auto --focus /RootNode -o before.png
usd-cli --session texture-after open <output_dir>/textured_output.usd
usd-cli --session texture-after render --renderer auto --focus /RootNode -o after.png
```

`render --focus` authors only session camera state in this inspection flow; do
not save or export either scene.
`auto` selects only a configured local or remote OVRTX placement; it does not
fall back to a software renderer. Use the root `$usd-cli` skill and its live
command help when changing sessions, cameras, or renderer placement.
