...
description: "Fixed pipeline reference: Run the Validation Agent CLI for template-based, config-driven, Python-compatible, or CI validation reports. Use when the user explicitly requests validation-agent, its named look_right/physics_sane/physical_behavior/render_valid templates, or V1 artifacts; use content-workflow-cli validate for unqualified validation tasks."
version: "0.1.0"
...
# Fixed Pipeline Reference: Validation Agent CLI
...
- For an unqualified validation task, use `content-workflow-cli validate run`
  instead.
...
- Use a future service/deploy skill instead if product scope adds REST,
  OpenAPI, or hosted service surfaces. Validation Agent V1 remains the 0.6
  fixed-pipeline Research Preview baseline and is CLI/Python-contract only.
