...
"""Workflow-owned Scene Optimizer path correspondence helpers."""
...
def _geomsubset_paths_by_parent(original_usd_path: Path) -> dict[str, list[str]]:
    ...
    # Scene Optimizer correspondence keys use the composed instance-proxy path
    # when a split mesh lives below an instance. ``TraverseAll`` visits the
    # prototype but not those proxy paths, which made split GeomSubsets fall
    # back to invented ``Mesh_part_N`` source paths. Include instance proxies so
    # the split index can resolve to the actual source GeomSubset.
