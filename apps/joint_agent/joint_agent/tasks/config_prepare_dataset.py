...
"""Prepare dataset configuration task for Joint Agent."""
...
class PrepareDatasetConfigTask(Task):
    ...
    Output context keys:
        ...
        - include_scene_prim_path_context: Whether to include a bounded scene
          prim-path inventory in prompts
        - scene_prim_path_limit: Max scene prim paths to include
    """
    ...
    def run(
        ...
        include_prim_path_context = config.get("include_prim_path_context", True)
        ...
        context.update(
            {
                ...
                "prompts": prompts,
                "include_prim_path_context": include_prim_path_context,
                "include_stage_coordinate_context": include_stage_coordinate_context,
                "include_scene_prim_path_context": include_scene_prim_path_context,
                "scene_prim_path_limit": config.get("scene_prim_path_limit", 80),
                "include_geometric_context": include_geometric_context,
                "include_repetition_context": include_repetition_context,
                "repetition_signature_depth": config.get(
                    "repetition_signature_depth", 2
                ),
                "repetition_sibling_limit": config.get("repetition_sibling_limit", 5),
            }
        )
    ...
    def _load_config(self, context: dict[str, Any]) -> dict[str, Any]:
        ...
        config, _ = load_config_mapping_from_context(
            ...
            file_non_mapping_message=(
                "Prepare dataset configuration must be a dictionary"
            ),
        )
        return config
