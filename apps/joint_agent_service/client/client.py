...
class JointAgentClient:
    ...
    Endpoints:
      ...
      - GET  /artifacts/{session_id}/joint-rigger-output (generated USDZ)
      - GET  /artifacts/{session_id}/joint-rigger-diagnostics (diagnostics JSON)
      - GET  /artifacts/{session_id}/joint-rigger-validation (result JSON)
    ...
    def __init__(
        ...
        self._http.headers.update({"User-Agent": "joint-agent-client/1.0"})
        if self._token:
    ...
    def start_pipeline(
        ...
        apply_joint_rigger: bool = False,
        joint_rigger_adapter: str | None = None,
        joint_rigger_on_missing_dependency: str | None = None,
        joint_rigger_on_unready_candidates: str | None = None,
        joint_rigger_template: str | None = None,
        joint_rigger_apply_masses: bool | None = None,
        joint_rigger_apply_collision: bool | None = None,
    ) -> str:
        """
        ...
        Args:
            ...
            apply_joint_rigger: Enable the opt-in Joint Rigger apply step.
            joint_rigger_adapter: Adapter name. When omitted, an enabled service
                defaults to the built-in topology-only ``owned_core`` adapter.
        ...
        if apply_joint_rigger:
            data["apply_joint_rigger"] = "true"
        if joint_rigger_adapter:
            data["joint_rigger_adapter"] = joint_rigger_adapter
        if joint_rigger_on_missing_dependency:
            data["joint_rigger_on_missing_dependency"] = (
                joint_rigger_on_missing_dependency
            )
        if joint_rigger_on_unready_candidates:
            data["joint_rigger_on_unready_candidates"] = (
                joint_rigger_on_unready_candidates
            )
        if joint_rigger_template:
            data["joint_rigger_template"] = joint_rigger_template
        if joint_rigger_apply_masses is not None:
            data["joint_rigger_apply_masses"] = str(joint_rigger_apply_masses).lower()
        if joint_rigger_apply_collision is not None:
            data["joint_rigger_apply_collision"] = str(
...
def build_arg_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Joint Agent Service client")
    parser.add_argument(
    ...
    parser.add_argument(
        "--apply-joint-rigger",
        action="store_true",
        help="Enable the opt-in Joint Rigger apply step",
    )
    parser.add_argument(
        "--joint-rigger-adapter",
        default=None,
        choices=["owned_core", "mock", "usd_joint_rigger"],
        help=(
            "Joint Rigger adapter to use when --apply-joint-rigger is set "
            "(default: owned_core)"
        ),
    )
    parser.add_argument(
        "--joint-rigger-on-missing-dependency",
        default=None,
        choices=["skip", "block"],
        help="Missing dependency policy for the Joint Rigger adapter",
    )
    parser.add_argument(
        "--joint-rigger-on-unready-candidates",
        default=None,
        choices=["warn", "block"],
        help="Candidate readiness policy for the Joint Rigger adapter",
    )
    parser.add_argument(
        "--joint-rigger-template",
        default=None,
        help="Template name for the provisional USD Joint Rigger",
    )
    masses = parser.add_mutually_exclusive_group()
    masses.add_argument(
        "--joint-rigger-apply-masses",
        dest="joint_rigger_apply_masses",
        action="store_true",
        help="Ask the Joint Rigger to author masses when supported",
    )
    masses.add_argument(
        "--no-joint-rigger-apply-masses",
        dest="joint_rigger_apply_masses",
        action="store_false",
        help="Disable Joint Rigger mass authoring",
    )
    collision = parser.add_mutually_exclusive_group()
    collision.add_argument(
        "--joint-rigger-apply-collision",
        dest="joint_rigger_apply_collision",
        action="store_true",
        help="Ask the Joint Rigger to author collision when supported",
    )
    collision.add_argument(
        "--no-joint-rigger-apply-collision",
        dest="joint_rigger_apply_collision",
        action="store_false",
        help="Disable Joint Rigger collision authoring",
    )
    parser.set_defaults(
...
def main(argv: list[str] | None = None) -> int:
    ...
    session_id, status = client.run_and_monitor(
        ...
        apply_joint_rigger=args.apply_joint_rigger,
        joint_rigger_adapter=args.joint_rigger_adapter,
        joint_rigger_on_missing_dependency=args.joint_rigger_on_missing_dependency,
        joint_rigger_on_unready_candidates=args.joint_rigger_on_unready_candidates,
        joint_rigger_template=args.joint_rigger_template,
        joint_rigger_apply_masses=args.joint_rigger_apply_masses,
        joint_rigger_apply_collision=args.joint_rigger_apply_collision,
