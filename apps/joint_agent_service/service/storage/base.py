...
def _prune_local_snapshot(
    ...
    """Remove local entries in ``prefix`` that are absent from the source."""

    entries = list(local_dir.rglob("*"))
    for entry in entries:
        if not (entry.is_file() or entry.is_symlink()):
            continue
        relative_key = entry.relative_to(local_dir).as_posix()
        if prefix and not relative_key.startswith(prefix):
            continue
        if relative_key not in source_relative_keys:
            entry.unlink(missing_ok=True)

    normalized_prefix = prefix.rstrip("/")
    directories = sorted(
        (entry for entry in entries if entry.is_dir() and not entry.is_symlink()),
        key=lambda entry: len(entry.relative_to(local_dir).parts),
        reverse=True,
    )
    for directory in directories:
        relative_dir = directory.relative_to(local_dir).as_posix()
        if prefix and not (
            relative_dir == normalized_prefix or relative_dir.startswith(prefix)
        ):
            continue
        try:
            directory.rmdir()
            ...
            if exc.errno not in {errno.EEXIST, errno.ENOTEMPTY}:
                raise
            # Non-empty directories remain part of the refreshed snapshot.
...
class SessionStore(Protocol):
    ...
    async def put_bytes_if_absent(
        ...
        """Atomically create a key, returning false when it already exists."""
        ...  # pragma: no cover
    ...
    async def compare_and_swap_bytes(
        ...
        """Atomically replace or delete a key only when its bytes still match."""
        ...  # pragma: no cover
    ...
    async def sync_to_local(
        ...
        *,
        ...
        Args:
            ...
            overwrite: Replace local files that already use the same key.
    ...
    async def sync_from_local(
        ...
        *,
        ...
        Args:
            ...
            overwrite: Replace remote files that already use the same key.
