...
def test_event_listener_maps_lifecycle_and_workflow_events(tmp_path: Path) -> None:
    ...
    assert workflow_completed.extra["workflow_completed"] is True
...
async def test_event_listener_event_schedules_on_loop() -> None:
    ...
    listener.event("task.started", {"task_name": "CustomTask"})
    ...
    assert snapshot["current_step"]["name"] == "CustomTask"
