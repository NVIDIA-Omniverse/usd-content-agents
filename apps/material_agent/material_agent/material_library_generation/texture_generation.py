...
def generate_texture_maps(
    ...
    source_albedo_path: str | Path | None = None,
) -> TextureMapSet:
    """Generate albedo/normal/ORM maps for one material recipe.

    A pre-generated albedo artifact may be supplied by an agentic image-generation
    skill. Otherwise, when an image model or backend is supplied, only the albedo
    map is generated by that model in this MVP. Normal and ORM maps are synthesized
    from PBR hints so the package remains deterministic and renderer-safe. This
    keeps the workflow as direct material-library synthesis rather than high-level
    Texture Agent texture projection.
    """
    ...
    if source_albedo_path is not None and (image_model is not None or settings.backend):
        raise ValueError(
            "source_albedo_path cannot be combined with an image model or backend"
        )

    if source_albedo_path is not None:
        source_path = Path(source_albedo_path).resolve()
        if not source_path.is_file():
            raise FileNotFoundError(f"Source albedo does not exist: {source_path}")
        with Image.open(source_path) as source_image:
            albedo = source_image.convert("RGB")
        if albedo.size != (settings.texture_size, settings.texture_size):
            albedo = albedo.resize(
                (settings.texture_size, settings.texture_size),
        ...
        if model is not None:
            ...
            try:
                ...
                albedo = _synthesize_albedo(
                    recipe, settings.texture_size, settings.seed
                )
        else:
            albedo = _synthesize_albedo(recipe, settings.texture_size, settings.seed)

    if settings.color_correct_albedo and source_albedo_path is None:
