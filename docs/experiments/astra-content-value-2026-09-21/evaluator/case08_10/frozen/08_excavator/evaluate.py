...
def evaluate(args):
    ...
    try:structural,config=inspect(args.usd,bindings,contract,inventory,source_root,args.inventory.parent,args.output);checks.extend(structural);checks.extend(inspect_extra(config,bindings))
    ...
    if not unsafe and not args.structural_only:
        for seed in SEEDS:
            ...
            with log_file.open('w') as log:
                proc=subprocess.run([str(args.solver_python),str(HERE/'solver.py'),'--config',str(args.output/'runtime_config.json'),'--seed',str(seed),'--output',str(result_file),'--device',args.device],stdout=log,stderr=subprocess.STDOUT,timeout=900)
