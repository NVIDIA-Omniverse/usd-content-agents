        ...
        if structural['pass']:
            for seed in SPEC['seeds']:
                ...
                with (trial/'solver.log').open('w') as log:
                    p=subprocess.run([solver,str(HERE/'drawer_solver.py'),str(trial/'request.json')],stdout=log,stderr=subprocess.STDOUT,env={**os.environ,'TMPDIR':'/opt/astra-content-value-20260921/tmp','UV_NO_CACHE':'1'},timeout=300)
...
if __name__=='__main__':
    ap=argparse.ArgumentParser();ap.add_argument('--bindings',type=pathlib.Path,required=True);ap.add_argument('--output',type=pathlib.Path,required=True);ap.add_argument('--solver',default='/opt/astra-content-value-20260921/ovphysx-venv/bin/python');args=ap.parse_args()
