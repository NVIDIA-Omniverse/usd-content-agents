...
ap=argparse.ArgumentParser();ap.add_argument('--trial',type=int);args=ap.parse_args()
OUT=Path(__file__).resolve().parent/'gravity_origin_probe_r3';OUT.mkdir(parents=True,exist_ok=True)
...
rows=[]
trials=[('far_constrained128',[1000,1000,1000],None,128),('near_constrained128',[1,1,1],None,128)]
if args.trial is None:
 for i in range(len(trials)):
  subprocess.run([sys.executable,str(Path(__file__).resolve()),'--trial',str(i)],check=True)
 rows=[json.loads((OUT/(t[0]+'.json')).read_text()) for t in trials]
 result={'scope':'Synthetic unscored evaluator-witness origin/iteration diagnostic; no author asset or frozen evaluator changed','runtime':'ovphysx0.4.13 native CPU; one SDK lifecycle per child process','dt_s':1/240,'steps':24,'created_utc':datetime.datetime.now(datetime.timezone.utc).isoformat(),'script_sha256':hashlib.sha256(Path(__file__).read_bytes()).hexdigest(),'trials':rows}
 (OUT/'results.json').write_text(json.dumps(result,indent=2)+'\n')
 sys.exit(0)
for name,pos,it,companion in [trials[args.trial]]:
 scene=OUT/(name+'.usda')
 content='''#usda 1.0
...
(OUT/(trials[args.trial][0]+'.json')).write_text(json.dumps(rows[0],indent=2)+'\n')
