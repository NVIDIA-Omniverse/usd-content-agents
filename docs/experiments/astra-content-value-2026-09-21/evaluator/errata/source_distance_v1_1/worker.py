...
def discover(root,case,run_id):
    if case=='02_conveyor':
        snapshot=root/'evaluator/conveyor'
        candidates=[snapshot/'reference/source_inventory.json',snapshot/'references/source_inventory.json',snapshot/'source_inventory.json']
        inventory=next((p for p in candidates if p.is_file() and all((p.parent/x['geometry_file']).is_file() for x in load(p)['parts'])),snapshot/'source_inventory.json')
    else:
...
def adjudicate(args):
    ...
    submission=load(run/'submission.json') if (run/'submission.json').is_file() else {}
    v1=load(v1dir/'acceptance.json')
    if any(not isinstance(submission.get(key),str) or not submission[key].strip() for key in ['final_scene','bindings']):
        assert not v1.get('accepted'),'A non-submission cannot have an accepted original verdict'
        out.mkdir(parents=True)
        receipt=copy.deepcopy(v1)
        receipt.update(version=VERSION,case_id=args.case,arm=args.arm,adjudication_action='skipped_non_submission',started_utc=started,finished_utc=utc(),worker_sha256=sha(__file__),qualification_sha256=sha(args.qualification),v1_acceptance_sha256=sha(v1dir/'acceptance.json'),author_manifest_sha256=sha(run/'output_manifest.json'),source_surface_checks_replaced=0,claimed_accepted=submission.get('claimed_accepted'),false_pass=bool(submission.get('claimed_accepted') is True and v1.get('concrete_failures')),input_scene_sha256=None,bindings_sha256=None,native_physics_origin='not_applicable_no_submitted_scene_or_bindings',tolerances_changed=False,frozen_files_modified=False,scored_artifacts_modified=False,skip_reason='No nonempty scene/bindings pair was submitted. Original contractual failure is preserved; no geometry or physics verdict is invented.')
        dump(out/'acceptance.json',receipt)
        conservative=copy.deepcopy(v1);conservative.update(version='v1_conservative_after_distance_erratum',case_id=args.case,arm=args.arm,v1_acceptance_sha256=receipt['v1_acceptance_sha256'],adjudication_action='skipped_non_submission',false_pass=receipt['false_pass'])
        dump(out/'conservative_v1.json',conservative)
        dump(out/'provenance.json',{'version':VERSION,'adjudication_action':'skipped_non_submission','hashes':{p.name:sha(p) for p in out.iterdir() if p.is_file()},'worker_sha256':sha(__file__)})
        print(json.dumps({'case':args.case,'arm':args.arm,'status':receipt['status'],'adjudication_action':'skipped_non_submission','output':str(out)}),flush=True)
        return
    ...
    from pxr import Usd
    source=load(inventory);bindings_path=None;manifest={x['path']:x for x in load(run/'output_manifest.json')['files']};inputs={}
...
def watch_all(args):
    ...
    while pending:
        ...
        for case,arm in sorted(pending):
            ...
            submission=load(run/'submission.json') if (run/'submission.json').is_file() else {}
            non_submission=any(not isinstance(submission.get(k),str) or not submission[k].strip() for k in ['final_scene','bindings'])
            if not non_submission and (not snapshot.is_dir() or not inventory.is_file()):continue
...
if __name__=='__main__':
    p=argparse.ArgumentParser();p.add_argument('--root',type=Path,default=Path('/opt/astra-content-value-20260921'));p.add_argument('--run-id',default='pilot-v1');p.add_argument('--case',choices=CASES+['all'],default='all');p.add_argument('--arm',choices=['plain_astra','content_agents','both'],default='both');p.add_argument('--snapshot',type=Path);p.add_argument('--inventory',type=Path);p.add_argument('--source-root',type=Path);p.add_argument('--v1-output',type=Path);p.add_argument('--output',type=Path);p.add_argument('--qualification',type=Path,default=HERE/'qualification_r2.json');p.add_argument('--qualify',action='store_true');p.add_argument('--wait-seconds',type=float,default=0);p.add_argument('--solver-python',type=Path);p.add_argument('--fresh-conveyor-if-needed',action='store_true');args=p.parse_args()
