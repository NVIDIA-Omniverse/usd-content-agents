...
class State:
    def __init__(self, sdk, path, moving):
        ...
        self.bindings = {}
        names = ['RIGID_BODY_POSE','RIGID_BODY_VELOCITY','RIGID_BODY_MASS','RIGID_BODY_INERTIA']
...
def schedule(contract, role, q0, t, joint):
    ...
    if joint.get('upper') is not None and joint['upper']>=joint.get('lower',-1e30):
        return joint['upper']+(0.3 if joint['kind']=='revolute' else 0.01)
    return q0
    ...
    try:
        ...
        for step in range(count):
            ...
            for path,joint in config['joints'].items():
                ...
                effort=float(np.clip(setting['kp']*(target-unwrapped[path])-setting['kd']*qdot[path],-setting['max_effort'],setting['max_effort']))*effort_jitter
                external=0.
                if 5.5<=t<6.5:external=-.01*setting['max_effort']*float(config.get('external_load_scale',1.0))
                # Small reproducible initial external perturbation across five seeds.
                if t<.05:external+=float(rng.uniform(-.005,.005))*setting['max_effort']
        ...
        result={'seed':seed,'completed_steps':step+1,'expected_steps':count,'direct_solver':'ovphysx.PhysX','device':device,
                'only_control_writes':['RIGID_BODY_FORCE','RIGID_BODY_WRENCH'],'native_mass':native_mass,'native_inertia':native_inertia,
