    ...
    try:
        ...
        for step in range(count):
            ...
            for path,joint in config['joints'].items():
                ...
                if setting.get('gain_mode')=='inertia_scaled':load=min(load,gain_scale*5.0)
                ...
                if t<.05:external+=float(rng.uniform(-.005,.005))*min(setting['max_effort'],gain_scale*5.0 if setting.get('gain_mode')=='inertia_scaled' else setting['max_effort'])
