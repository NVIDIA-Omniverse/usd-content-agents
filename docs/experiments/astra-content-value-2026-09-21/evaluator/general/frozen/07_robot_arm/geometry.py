...
def load_fcstd(path, unit):
    ...
    def walk(docpath,name,prefix,ancestors):
        ...
        if typ=='App::Link':
        ...
        elif typ in ('App::DocumentObjectGroup','App::FeaturePython','PartDesign::CoordinateSystem','PartDesign::Line','PartDesign::Plane','App::Origin','Sketcher::SketchObject','App::Line','App::Plane'):
            return
            ...
            part=part_element.get('file');shape_key=(str(docpath),part,unit)
            if shape_key not in mesh_cache:
                shape=TopoDS_Shape()
                with zipfile.ZipFile(docpath) as z:BRepTools.Read_s(shape,io.BytesIO(z.read(part)),BRep_Builder())
                from OCP.TopExp import TopExp_Explorer
                from OCP.TopAbs import TopAbs_FACE
                if not TopExp_Explorer(shape,TopAbs_FACE).More():
                    print('Excluded non-surface construction geometry: '+str(docpath)+':'+prefix+':'+typ,flush=True);return
                try:mesh_cache[shape_key]=occ_mesh(shape,unit)
                except Exception as exc:raise ValueError(f'{docpath}:{prefix}:{typ}: {exc}') from exc
            vertices,faces=mesh_cache[shape_key]
...
def surface_compare(source_v, source_f, final_v, final_f, tolerance):
    """Symmetric deterministic surface samples plus source/final vertex coverage."""
    a = trimesh.Trimesh(source_v, source_f, process=False)
    b = trimesh.Trimesh(final_v, final_f, process=False)
    rng = np.random.default_rng(101)
    def samples(mesh):
        # Include all vertices, not only samples that might miss thin details.
        idx = rng.choice(len(mesh.faces), size=min(12000,max(2000,len(mesh.faces))), p=mesh.area_faces/mesh.area)
        uv = rng.random((len(idx),2))
        uv[uv.sum(1)>1] = 1-uv[uv.sum(1)>1]
        tri = mesh.triangles[idx]
        p = tri[:,0]+uv[:,0,None]*(tri[:,1]-tri[:,0])+uv[:,1,None]*(tri[:,2]-tri[:,0])
        return np.vstack([mesh.vertices, p])
    # Batch calls to keep memory bounded on detailed CAD assemblies.
    def max_distance(points, mesh):
        result = 0.0
        for start in range(0,len(points),2000):
            _, dist, _ = trimesh.proximity.closest_point(mesh, points[start:start+2000])
            result = max(result,float(np.max(dist)))
        return result
    max_ab, max_ba = max_distance(samples(a),b), max_distance(samples(b),a)
    area_ratio = b.area/a.area
    bounds_error = float(np.max(np.abs(a.bounds-b.bounds)))
    passed = max(max_ab,max_ba) <= tolerance and abs(area_ratio-1) <= 0.03 and bounds_error <= tolerance
    return bool(passed), {'source_to_final_max_sample_distance_m':max_ab, 'final_to_source_max_sample_distance_m':max_ba,
                          'area_ratio':float(area_ratio),'bounds_error_m':bounds_error,'tolerance_m':tolerance,
                          'method':'all vertices + deterministic area-weighted surface samples, symmetric nearest triangle distances; 3% area tolerance; no topology-preservation claim'}
