...
def inspect(final_usd, bindings, contract, source_inventory, source_root, inventory_root, output):
    ...
    for f in source_inventory['source_files']:
        ...
        check(checks,'source_bytes:'+f['path'],p.is_file() and sha(p)==f['sha256'])
    ...
    for role,expected_roles in contract.get('joint_role_endpoints',{}).items():
        if role in joint_roles and all(r in body_roles for r in expected_roles):
            joint=joints[joint_roles[role]];check(checks,'joint_source_link_endpoints:'+role,{joint['body0'],joint['body1']}=={body_roles[r] for r in expected_roles})
    ...
    if contract.get('require_joint_graph_cycle'):
        ...
        intended={body_roles[r] for r in contract['required_body_roles'] if r in body_roles}
        cyclic=any(intended<=c and sum(a in c and b in c for a,b in edges)>=len(c) for c in components)
        check(checks,'actual_joint_graph_has_closed_loop',cyclic)
        for role in contract.get('required_cycle_joint_roles',[]):
            if role not in joint_roles:continue
            path=joint_roles[role];edge=joints[path];seen={edge['body0']};front=[edge['body0']]
            while front:
                node=front.pop()
                for other_path,other in joints.items():
                    if other_path==path:continue
                    for a,b in [(other['body0'],other['body1']),(other['body1'],other['body0'])]:
                        if a==node and b not in seen:seen.add(b);front.append(b)
            check(checks,'required_joint_is_in_closed_loop:'+role,edge['body1'] in seen)
    ...
    for part in source_inventory['parts']:
        ...
        check(checks,'frozen_geometry_hash:'+ident,sha(pfile)==part['geometry_sha256'])
        ...
        for mesh_path in paths:
            cursor=stage.GetPrimAtPath(mesh_path);ancestors=[]
            while cursor and not cursor.IsPseudoRoot():
                if cursor.HasAPI(UsdPhysics.RigidBodyAPI) and UsdPhysics.RigidBodyAPI(cursor).GetRigidBodyEnabledAttr().Get():ancestors.append(str(cursor.GetPath()))
                cursor=cursor.GetParent()
            expected_body=mapping['body_path'];moving=bodies.get(expected_body,{}).get('moving',False)
            check(checks,'source_actual_rigid_ancestry:'+mesh_path,ancestors==[expected_body] if moving else not any(bodies.get(a,{}).get('moving',False) for a in ancestors),ancestors)
    ...
    # Keep the frozen external controller as the only active actuator.
    authored_forces=[]
    for prim in stage.Traverse():
        for a in prim.GetAttributes():
            name=a.GetName()
            if name.startswith('drive:') and name.endswith((':stiffness',':damping')):a.Set(0.)
            if name.startswith('physxForce:') and name.endswith((':force',':torque')) and a.Get() is not None and np.any(np.asarray(a.Get())!=0):authored_forces.append(str(a.GetPath()))
        if prim.HasAPI(UsdPhysics.RigidBodyAPI):
            rb=UsdPhysics.RigidBodyAPI(prim);rb.CreateVelocityAttr(Gf.Vec3f(0));rb.CreateAngularVelocityAttr(Gf.Vec3f(0))
    check(checks,'no_authored_external_forces',not authored_forces,authored_forces)
    # Evaluator-owned free-fall witness, remote from asset; never writes original.
    witness='/__EvaluatorGravityWitness'
    cube=UsdGeom.Cube.Define(stage,witness);cube.CreateSizeAttr(.01)
    cube.AddTranslateOp().Set(Gf.Vec3d(1000,1000,1000))
