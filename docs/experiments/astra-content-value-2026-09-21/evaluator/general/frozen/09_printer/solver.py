...
def run(config, seed, output, device='cpu'):
    ...
    try:
        ...
        controller_scales={};controller_descendants={};controller_masses={}
        for path,joint in config['joints'].items():
            ...
            controller_masses[path]=sum(native_mass[body][0] for body in reached if body in states)
            if config.get('case09') and config['case09']['bed_path'] in reached:controller_masses[path]+=native_mass[config['case09']['payload_path']][0]
        contact_binding=None;contact_samples=[]
        if config.get('case09'):
            cc=config['case09'];contact_binding=sdk.create_contact_binding(sensor_patterns=[cc['payload_path']],filter_patterns=[cc['bed_path']],filters_per_sensor=1,max_contact_data_count=128)
        ...
        for step in range(count):
            ...
            for path,joint in config['joints'].items():
                ...
                gain_scale=controller_scales.get(path,1.) if setting.get('gain_mode')=='inertia_scaled' else (controller_masses.get(path,1.) if setting.get('gain_mode')=='mass_scaled' else 1.)
                gravity_ff=0.
                ...
                if setting.get('gravity_compensate') and joint['kind']=='prismatic' and config.get('case09') and config['case09']['bed_path'] in controller_descendants.get(path,set()):gravity_ff+=9.81*native_mass[config['case09']['payload_path']][0]*measure['axis_world'][2]
                ...
                if setting.get('gain_mode')=='mass_scaled':load=min(load,gain_scale*.5)
            ...
            if contact_binding is not None:
                matrix=np.zeros((1,1,3),np.float32);contact_binding.read_force_matrix(matrix)
                contact_samples.append({'t':(step+1)*dt,'impulse_world':matrix.reshape(-1).astype(float).tolist(),'payload_pose':states[cc['payload_path']].read('RIGID_BODY_POSE').tolist(),'bed_pose':states[cc['bed_path']].read('RIGID_BODY_POSE').tolist()})
            ...
            if step%12==0 or step==count-1:
                trace.append({'t':t,'poses':poses,'velocities':velocities,'joints':measures,'contact_headers':report['num_headers']})
            if not all_finite or max_linear>common['max_linear_speed_m_s'] or max_angular>common['max_angular_speed_rad_s'] or max_closure>common['max_joint_closure_m'] or max_axis>common['max_joint_axis_error_rad'] or max_penetration>common['max_penetration_m']:break
        result={'seed':seed,'completed_steps':step+1,'expected_steps':count,'direct_solver':'ovphysx.PhysX','device':device,
                'only_control_writes':['RIGID_BODY_FORCE','RIGID_BODY_WRENCH'],'native_mass':native_mass,'native_inertia':native_inertia,'native_com':native_com,'controller_effective_inertia':controller_scales,'controller_effective_mass':controller_masses,'case09_contact_samples':contact_samples,
        ...
        dump(output,result)
        if contact_binding is not None:contact_binding.destroy()
        witness.close()
    finally:
