...
def inside():
 ...
 base=['/tools/bin/usd-cli','--json','--timeout','600'];result={'scope':'Synthetic actual same-session render readiness followed by explicit CPU Physics; no model/benchmark acceptance.'}
...
def run(a):
 ...
 end=max(i for i,x in enumerate(argv)if x=='--');argv=argv[:end+1]+['/tools/main-venv/bin/python','-B','/work/probe.py','--inside']
