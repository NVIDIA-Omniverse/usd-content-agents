...
def evaluate(a):
 if not author_input_preflight(a,HERE):return
 ...
 try:
  ...
  structural,config=inspect_with_deadline(inspect,a.usd,bindings,contract,inventory,Path(a.source_root),a.inventory.parent,a.output);checks+=structural
  if config:
   ...
   if all(c['passed'] for c in checks):
    for seed in spec['seeds']:
     ...
     with (trial/'solver.log').open('w') as log:run_isolated([a.solver_python,str(HERE/'solver.py'),str(trial/'request.json')],stdout=log,stderr=subprocess.STDOUT,timeout=300)
     ...
     if result['status']=='INCONCLUSIVE':check(checks,f'seed{seed}:solver',False,result.get('error'),True)
      ...
      for name,passed in result['checks'].items():check(checks,f'seed{seed}:'+name,passed,insufficient=(name=='independent_gravity'))
 except Exception as exc:check(checks,'evaluator_completed',False,{'error':repr(exc),'traceback':traceback.format_exc()},True)
 checks=classify_uncalibrated_runtime(checks);report=verdict(checks);report.update(case_id='02_conveyor',source_inventory_sha256=sha(a.inventory),scene_sha256=sha(a.usd),bindings_sha256=sha(a.bindings),scope=spec['scope'])
