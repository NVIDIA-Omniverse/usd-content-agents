from v2_verdict import classify_uncalibrated_runtime, witness_check
from v2_process import run as run_isolated, inspect_with_deadline
...
from case_hooks import inspect_extra,observe_extra
...
def observe(result,config):
    ...
    if result['completed_steps']!=result['expected_steps']:
        return checks # Recorded direct-run/bound failures; later time windows do not exist.
    ...
    chk('independent_freefall_witness',abs(vz-expected)<.03 and abs(dz+.5*9.81*w['t']**2)<.004,{'time':w['t'],'delta_z':dz,'vz':vz,'expected_vz':expected},True)
    ...
    if observer=='door':
        ...
        if observer=='arm':
            ee=config['body_roles'][contract.get('end_effector_role','end_effector')];points=np.array([row['poses'][ee][:3] for row in trace]);travel=float(np.max(np.linalg.norm(points-points[0],axis=1)))
    ...
    checks.extend(observe_extra(result,config))
    return checks

def evaluate(args):
    if not author_input_preflight(args,HERE):return
    ...
    if frozen_path.exists():
        ...
        check(checks,'reference_inventory_frozen',sha(args.inventory)==frozen['reference_inventory_sha256'],insufficient=True)
    ...
    try:structural,config=inspect_with_deadline(inspect,args.usd,bindings,contract,inventory,source_root,args.inventory.parent,args.output);checks.extend(structural);checks.extend(inspect_extra(config,bindings))
    ...
    if not unsafe and not args.structural_only:
        for seed in SEEDS:
            ...
            with log_file.open('w') as log:
                proc=run_isolated([str(args.solver_python),str(HERE/'solver.py'),'--config',str(args.output/'runtime_config.json'),'--seed',str(seed),'--output',str(result_file),'--device',args.device],stdout=log,stderr=subprocess.STDOUT,timeout=900)
            if not result_file.exists():
                check(checks,f'seed{seed}:solver_process',False,{'returncode':proc.returncode,'log':str(log_file)},True);continue
            checks.extend(observe(json.loads(result_file.read_text()),config))
    ...
    if args.case=='06_engine' and not unsafe and not args.structural_only:
        ...
        with (args.output/'unloaded_seed11.log').open('w') as log:
            run_isolated([str(args.solver_python),str(HERE/'solver.py'),'--config',str(baseline_config),'--seed','11','--output',str(result_file),'--device',args.device],stdout=log,stderr=subprocess.STDOUT,timeout=900)
        ...
        if result_file.exists() and loaded_path.exists():
            ...
            if 'trace' in unloaded and 'trace' in loaded:
                ...
                check(checks,'engine_paired_load_response',bool(differences) and max(differences)>=1e-4,{'max_angle_change_rad':max(differences) if differences else None,'required_rad':1e-4,'input':'same seed, scene and controller; 0.02 Nm external shaft torque removed only in paired baseline'})
    ...
    checks=classify_uncalibrated_runtime(checks);report=verdict(checks)
