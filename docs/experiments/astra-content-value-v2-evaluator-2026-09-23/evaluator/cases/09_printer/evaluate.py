...
def evaluate(args):
    ...
    try:structural,config=inspect_with_deadline(inspect,args.usd,bindings,contract,inventory,source_root,args.inventory.parent,args.output);checks.extend(structural)
    ...
    if config and args.case=='09_printer':
        from case09_payload import prepare as prepare09
        try:
            if not prepare09(config,bindings,inventory,checks,args.output):config=None
        except Exception as exc:
            check(checks,'case09_preparation_completed',False,{'error':str(exc),'traceback':traceback.format_exc()},True);config=None
    ...
    if not unsafe and not args.structural_only:
        for seed in SEEDS:
            ...
            with log_file.open('w') as log:
                proc=run_isolated([str(args.solver_python),str(HERE/('case04_solver.py' if args.case=='04_gripper' else 'solver.py')),'--config',str(args.output/'runtime_config.json'),'--seed',str(seed),'--output',str(result_file),'--device',args.device],stdout=log,stderr=subprocess.STDOUT,timeout=900)
            ...
            if args.case=='04_gripper':
                ...
                checks.extend(observe(result,config))
                if args.case=='09_printer':
                    from case09_payload import observe as observe09
                    checks.extend(observe09(result,config))
