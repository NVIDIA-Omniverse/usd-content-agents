...
def solve(req):
    ...
    try:
        ...
        dp.read(dpos);pp.read(ppos)
        initial=dpos[0].astype(float).copy();authored=np.array(req['initial_drawer_pose'])
        initialerror=float(np.linalg.norm(initial[:3]-authored[:3]))
        initialrotation=quaternion_angle_deg(initial[3:7],authored[3:7])
        result.update(initial_drawer_pose=initial.tolist(),initial_payload_pose=ppos[0].tolist(),import_pose_error_m=initialerror,import_rotation_error_deg=initialrotation)
        tolerance=s['initial_pose_tolerance']
        if initialerror>tolerance['translation_m'] or initialrotation>tolerance['rotation_deg']:
            raise RuntimeError('Native initial body pose differs from authored pose')
        ...
        with (out/'trace.jsonl').open('w') as trace:
            for i in range(n):
                ...
                q=float(dpos[0,2]-initial[2]);off=float(np.linalg.norm(dpos[0,:2]-initial[:2]));rot=quaternion_angle_deg(dpos[0,3:7],initial[3:7])
                speed=max(np.linalg.norm(dvel[0,:3]),np.linalg.norm(pvel[0,:3]));spin=max(np.linalg.norm(dvel[0,3:]),np.linalg.norm(pvel[0,3:]));penetration=max([0.0]+[-x['separation'] for x in contacts]);contact_impulse=float(np.linalg.norm(cf))
                ...
                relative=payload_in_initial_world(ppos[0,:3],dpos[0],initial);payload=s['payload']
