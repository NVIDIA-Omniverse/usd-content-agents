...
result={'scope':'Synthetic source-parenting mutation tests, not benchmark author outputs','pass':all(x['pass'] for x in checks),'tests':checks};E.write_json(ROOT/'static_binding_test_evidence.json',result);print(json.dumps(result,indent=2));assert result['pass']
