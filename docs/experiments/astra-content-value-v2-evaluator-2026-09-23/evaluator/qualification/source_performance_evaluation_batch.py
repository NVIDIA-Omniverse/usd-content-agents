...
for name,command in jobs:
 t=time.monotonic()
 with (a.output/(name+'.log')).open('w') as log:proc=subprocess.run(['/usr/bin/timeout','--kill-after=5s','1800s']+command,stdout=log,stderr=subprocess.STDOUT)
 path=a.output/name/'result.json';result=json.loads(path.read_text()) if path.is_file() else {}
 row={'name':name,'returncode':proc.returncode,'elapsed_s':time.monotonic()-t,'wall_cap_s':1800,'qualified':proc.returncode==0 and result.get('passed') is True,'result':str(path),'disposition':'completed_source_measurement' if proc.returncode==0 else 'INCONCLUSIVE_SOURCE_QUALIFICATION'};rows.append(row);print(json.dumps(row),flush=True)
