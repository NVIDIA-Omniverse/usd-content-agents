...
def recompute(rows, request, trial):
    ...
    speeds=[]; spins=[]; drawer_speeds=[]; forces=[]; observed_forces=[]; raw_impulses=[]; penetrations=[]
    ...
    for i,r in enumerate(rows):
        ...
        forces.append(abs(fz));observed_forces.append(abs(float(f[2])))
    ...
    checks={'opening':metrics['min_open_hold_q_m']>=spec['opening_required_m'],
            ...
            'force_bounded':metrics['max_force_n']<=ctrl['force_limit_n']+1e-6 and max(observed_forces)<=ctrl['force_limit_n']+1e-6,
    ...
    source_open_min=min(source_q[i] for i in range(len(rows)) if hold_end-.5<=i*dt<hold_end)
    source_closed_max=max(abs(source_q[i]) for i in range(len(rows)) if total-.5<=i*dt)
    opening_interval=[source_open_min-import_error,source_open_min+import_error]
    closing_interval=[max(0.,source_closed_max-import_error),source_closed_max+import_error]
    ...
    return {'metrics':metrics,'reported_metric_comparison':comparisons,'trace_checks':checks,
            ...
            'max_observed_trace_force_n':max(observed_forces),'opening_source_frame_min_interval_m':opening_interval,'closing_source_frame_max_interval_m':closing_interval,'opening_gate_unambiguous_under_import_error':not(opening_interval[0]<spec['opening_required_m']<=opening_interval[1]),'closing_gate_unambiguous_under_import_error':not(closing_interval[0]<=spec['closure_tolerance_m']<closing_interval[1]),'max_source_frame_q_m':max(source_q),'max_normalized_authored_frame_rotation_deg':max(normalized_rotations),
...
def audit(args):
    ...
    inputs=[];first_bindings={};result={'schema_version':'capstone-independent-task-evidence-audit.v1','started_utc':datetime.datetime.now(datetime.timezone.utc).isoformat(),'script_sha256':sha(Path(__file__)),'status':'INCONSISTENT_OR_INCOMPLETE','audit_consistent':False,'task_acceptance_attested':False,'trials':[],'errors':[]}
    def bind(path):
        item=binding(path)
        if item['path'] in first_bindings:
            require(first_bindings[item['path']]==item,'Input changed between repeated closure reads: '+item['path'])
        else:first_bindings[item['path']]=item
    ...
    try:
        ...
        for seed,reported in zip(SEEDS,aggregate['trials']):
            ...
            require(all(computed[k] for k in ['off_axis_gate_unambiguous_under_import_error','payload_retention_unambiguous_under_import_error','opening_gate_unambiguous_under_import_error','closing_gate_unambiguous_under_import_error']),'Import translation uncertainty straddles a check threshold')
            ...
            require(type(tr['checks']['interior_clear']) is bool and tr['checks']['interior_clear']==interior,'Interior report contradicts retained query rows')
        ...
        all_pass=all(t['pass'] for t in aggregate['trials']);require(type(aggregate['pass']) is bool and aggregate['pass']==all_pass and aggregate['status']==('PASS' if all_pass else 'FAIL'),'Aggregate task verdict contradicts five reports')
    ...
    try:
        ...
        result['all_bound_input_bytes_unchanged']=False
