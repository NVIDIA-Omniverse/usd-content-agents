...
def check_geometry_handoff(path):
    """Retain Geometry's conditional handoff without inventing SimReady status."""
    from content_agent_workflows.common.validation_evidence import ValidationEvidence

    evidence = read(path)
    ValidationEvidence.model_validate(evidence)
    metadata = evidence.get("metadata", {})
    require(
        evidence.get("schema_version") == "content-agent-workflows.validation-evidence.v1"
        and evidence.get("workflow") == "geometry"
        and isinstance(metadata, dict)
        and metadata.get("schema_version") == "content-agent-workflows.geometry.v3"
        and evidence.get("sim_ready_status") == "not_evaluated"
        and metadata.get("handoff_ready") == "conditional",
        "Expected Geometry v3 conditional handoff with SimReady not_evaluated is missing or changed in meaning.",
    )
    return {
        "schema_version": metadata["schema_version"],
        "workflow": evidence["workflow"],
        "sim_ready_status": evidence["sim_ready_status"],
        "handoff_ready": metadata["handoff_ready"],
        "scope": "Historical conditional geometry handoff; no SimReady pass or waiver inferred.",
    }
...
def prepare(args):
    ...
    geometry_handoff = check_geometry_handoff(args.geometry_evidence)
    policy = {
